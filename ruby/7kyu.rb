...
def find_next_square(square)
  next_i = Math.sqrt(square).to_i + 1
  next_sq = next_i * next_i
  Math.sqrt(square).to_s[-1] == '0' ? next_sq : -1
end
# refactor
...
def sum_digits(number)
  number.to_s.split('').reduce(0) { |num| + num.to_i }
end
# refactor
...
def mxdiflg(a1, a2)
  ...
  a1 = a1.sort_by(&:length)
  a2 = a2.sort_by(&:length)
  if (a1[-1].length - a2[0].length).abs > (a1[0].length - a2[-1].length).abs
    (a1[-1].length - a2[0].length).abs
  else
    (a1[0].length - a2[-1].length).abs
  end
end
# refactor
